...
"""
function solve_pressure_correction!(p_corr::Array{Float64,2}, velocity::VectorFieldStaggered2D, ccfs::CorrectionCoefficients2D, props::FluidProperties, grid::GridRegular2D; max_itr::Int64=100, tol::Float64=1e-9)
    for itr in 1:max_itr
        ...
        for j in 2:grid.ny-1
            for i in 2:grid.nx-1
                ...
                p_corr[i, j] = (a_ip1 * u_E + a_im1 * u_W + a_jp1 * v_N + a_jm1 * v_S + b) / a_ij
...
function correct_velocity!(velocity::VectorFieldStaggered2D, tentative_velocity::VectorFieldStaggered2D, p_corr::Array{Float64,2}, ccfs::CorrectionCoefficients2D, grid::GridRegular2D, relax::RelaxationFactors)
    for j in 2:grid.ny-1
        for i in 2:grid.nx
            velocity.u[i, j] = tentative_velocity.u[i, j] + relax.velocity * ccfs.d_u[i, j] * (p_corr[i-1, j] - p_corr[i, j])
        end
    end

    for j in 2:grid.ny
        for i in 2:grid.nx-1
            velocity.v[i, j] = tentative_velocity.v[i, j] + relax.velocity * ccfs.d_v[i, j] * (p_corr[i, j-1] - p_corr[i, j])
        end
    end
end
